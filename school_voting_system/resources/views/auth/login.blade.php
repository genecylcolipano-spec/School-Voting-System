        ...
        <div class="relative w-full max-w-md">
            <div class="mb-8 text-center">
                @if ($loginLogo = \App\Support\SchoolBranding::logoUrl(withFallback: false))
                    <img src="{{ $loginLogo }}" alt="{{ \App\Support\SchoolBranding::schoolName() }}" class="mx-auto h-16 w-16 rounded-2xl border border-white/10 object-cover shadow-lg shadow-cyan-900/30">
                @else
                ...
                @endif
                <p class="mt-4 text-xs font-semibold uppercase tracking-[0.35em] text-cyan-300/80">{{ \App\Support\SchoolBranding::systemName() }}</p>
                <h1 class="mt-3 text-3xl font-bold text-white">Secure Passkey Portal</h1>
                <p class="mt-2 text-sm text-slate-400">{{ \App\Support\SchoolBranding::periodLabel() }}</p>
                @if ($poweredBy = \App\Support\SchoolBranding::poweredBy())
                    <p class="mt-1 text-sm text-slate-500">{{ $poweredBy }}</p>
                @endif
            ...
            <div class="rounded-2xl border border-white/10 bg-white/5 p-8 shadow-2xl backdrop-blur-xl">
                ...
                <p class="mt-6 flex flex-col gap-2 text-center text-xs">
                    <a href="{{ route('register') }}" class="text-cyan-300 hover:text-cyan-200"> Create an account?</a>
                    <a href="{{ route('login.recovery') }}" class="text-slate-400 hover:text-slate-300">Lost your passkey? Recovery options</a>
                </p>
            </div>
