...
<div
    ...
    data-live-voting-url="{{ route('admin.dashboard.live-voting') }}"
    data-voting-starts="{{ $countdownStartsIso }}"
    data-voting-ends="{{ $countdownEndsIso }}"
    data-countdown-phase="{{ $countdownPhase }}"
    data-is-paused="{{ $isPaused ? '1' : '0' }}"
>
    {{-- Breadcrumb --}}
    <nav class="vm-fade-in mb-4 flex flex-wrap items-center gap-1.5 text-xs text-slate-500" aria-label="Breadcrumb">
        <a href="{{ route('admin.dashboard') }}" class="font-medium text-slate-400 transition hover:text-violet-300">Dashboard</a>
        ...
        <div class="grid gap-5 lg:grid-cols-12 lg:items-start">
            <div class="min-w-0 lg:col-span-7">
                ...
                <p id="live-voting-election-title" class="mt-1 truncate text-sm font-medium text-slate-200">
                    {{ $election?->title ?? 'No assigned election' }}
                </p>
            ...
            <div class="flex flex-col gap-3 lg:col-span-5 lg:items-end">
                ...
                <div class="w-full rounded-xl border border-violet-500/20 bg-slate-900/80 px-4 py-3 text-right lg:w-auto lg:min-w-[14rem]">
                    <p id="live-voting-countdown-label" class="text-[10px] font-semibold uppercase tracking-wide text-slate-500">{{ $countdownLabel }}</p>
    ...
    @if (! $election)
        <div class="vm-empty-state vm-slide-up mt-6 rounded-2xl border border-dashed border-slate-700 bg-slate-950/40">
            ...
            <a href="{{ route('admin.elections.index') }}" class="vm-btn vm-btn--ghost mt-5">Manage Elections</a>
        </div>
    @else
        ...
        <section class="vm-card vm-fade-in mt-5 p-4 sm:p-5" aria-label="Election information">
            ...
            <dl class="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                <div>
                    ...
                    <dd class="mt-1 text-sm font-medium text-white">{{ $election->title }}</dd>
                </div>
                <div>
                    <dt class="text-[10px] font-semibold uppercase tracking-wide text-slate-500">Category</dt>
                    <dd class="mt-1 text-sm font-medium text-white">{{ $electionCategory }}</dd>
                </div>
                <div>
                    <dt class="text-[10px] font-semibold uppercase tracking-wide text-slate-500">Voting Scope</dt>
                    <dd class="mt-1 text-sm font-medium text-white">{{ $assignedRole ?? 'Operations Admin' }}</dd>
                </div>
                <div>
                    ...
                    <p class="mt-0.5 text-xs text-slate-400">{{ $academicSemester }}</p>
                </div>
                <div>
        ...
        <div class="vm-fade-in mt-4 flex flex-wrap items-center gap-2 rounded-2xl border border-violet-500/10 bg-slate-950/40 p-3 sm:p-4" role="toolbar" aria-label="Election quick actions">
            @if ($canPauseElection)
                @if ($election->is_paused)
                    <form method="POST" action="{{ route('admin.election.resume', $election) }}" data-confirm-sensitive data-confirm-title="Resume election?">
                        @csrf
                        <button type="submit" class="vm-btn vm-btn--success" aria-label="Resume election">
                            ▶ Resume Election
                        </button>
                    </form>
                @elseif ($showLivePanel || $election->status?->value === 'active')
                    <form method="POST" action="{{ route('admin.election.pause', $election) }}" data-confirm-sensitive data-confirm-title="Pause election?">
                        @csrf
            ...
            <a href="{{ route('admin.results.election.show', $election) }}" class="vm-btn vm-btn--primary" aria-label="View results for {{ $election->title }}">
                🏆 View Results
            </a>

            @if ($canEditElection ?? false)
                <a href="{{ route('admin.elections.edit', $election) }}" class="vm-btn vm-btn--ghost" aria-label="Edit election settings">
                    ✏ Edit Election
                </a>
            @endif

            @if ($canExportPreliminary ?? false)
                <a href="{{ route('admin.results.election.export', ['election' => $election, 'format' => 'csv']) }}" class="vm-btn vm-btn--ghost" aria-label="Export results">
                    ⬇ Export Results
                </a>
            @endif
        </div>
        ...
        <div
            ...
            data-state="{{ $isPaused ? 'paused' : ($isOpen ? 'live' : ($isCompleted ? 'completed' : 'idle')) }}"
        >
            @if ($isOpen)
            ...
            @elseif ($isCompleted)
                <span class="vm-live-status__icon" aria-hidden="true">{{ $isResultsPublished ? '🏆' : '🔵' }}</span>
                <div>
                    <p class="font-semibold text-sky-200">{{ $isResultsPublished ? 'RESULTS PUBLISHED' : 'COMPLETED' }}</p>
    ...
    {{-- Idle / waiting state --}}
    <div id="live-voting-idle" class="vm-slide-up mt-6 flex flex-1 flex-col items-center justify-center rounded-2xl border border-dashed border-slate-700 bg-slate-950/40 px-4 py-12 text-center{{ $showLivePanel ? ' hidden' : '' }}">
        ...
        @if ($election)
            <a href="{{ route('admin.elections.edit', $election) }}" class="vm-btn vm-btn--ghost mt-5">Open election settings</a>
        @endif
    </div>

    {{-- Live dashboard --}}
    <div id="live-voting-active" class="vm-layout-stack mt-6{{ $showLivePanel ? '' : ' hidden' }}">
        {{-- Summary statistics --}}
        <div class="vm-layout-grid vm-layout-grid--stats">
            <article class="vm-stat-card vm-stat-card--enhanced vm-slide-up" style="animation-delay: 0.05s">
                ...
                <p id="live-voting-total-votes" class="vm-stat-card__value" data-animate-counter="{{ $liveVotesCast }}">{{ number_format($liveVotesCast) }}</p>
            ...
            <article class="vm-stat-card vm-stat-card--enhanced vm-slide-up" style="animation-delay: 0.1s">
                ...
                <p id="live-voting-turnout" class="vm-stat-card__value" data-animate-counter="{{ $liveTurnout }}">{{ number_format($liveTurnout, 1) }}%</p>
                <p id="live-voting-turnout-detail" class="vm-stat-card__meta">{{ number_format($liveUniqueVoters) }} / {{ number_format($liveEligible) }} Students</p>
            </article>
            ...
            <article class="vm-stat-card vm-stat-card--enhanced vm-slide-up" style="animation-delay: 0.15s">
                ...
                <p id="live-voting-stat-countdown" class="vm-stat-card__value tabular-nums">{{ $countdownRemaining }}</p>
                <p id="live-voting-stat-countdown-hint" class="vm-stat-card__meta">{{ $countdownHint }}</p>
            </article>
            ...
            <article class="vm-stat-card vm-stat-card--enhanced vm-slide-up" style="animation-delay: 0.2s">
                ...
                <p id="live-voting-registered" class="vm-stat-card__value" data-animate-counter="{{ $liveEligible }}">{{ number_format($liveEligible) }}</p>
        ...
        <footer class="vm-card vm-fade-in p-4 sm:p-5" aria-label="System status">
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div>
                    ...
                    <p id="live-voting-system-clock" class="mt-1 text-sm font-medium text-white tabular-nums">{{ now()->format('g:i A') }}</p>
                </div>
            </div>
        </footer>
    </div>
...
@push('styles')
    @vite('resources/css/admin-live-voting.css')
@endpush
