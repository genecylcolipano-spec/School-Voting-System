<x-app-layout>
    <x-admin-portal :title="($record ? 'Edit ' : 'Add ') . $rosterLabel . ' Roster'" :user="$user" :notifications-count="$notificationsCount">
        ...
        <section class="mx-auto max-w-2xl rounded-2xl border border-violet-500/15 bg-slate-900/70 p-5 sm:p-6">
            <form method="POST" action="{{ $record ? route($routePrefix.'.update', $record) : route($routePrefix.'.store') }}" class="space-y-5">
                @csrf
                @if ($record)
                    @method('PUT')
                @endif
                ...
                <div>
                    ...
                    @if ($accountIdLocked)
                        <input value="{{ $record->account_id }}" readonly disabled
                    ...
                    @else
                        <input name="account_id" value="{{ old('account_id', $record?->account_id) }}" required
                ...
                <div class="flex flex-wrap gap-3 pt-2">
                    <button type="submit" class="rounded-xl bg-gradient-to-r from-violet-600 to-indigo-500 px-5 py-2.5 text-sm font-semibold text-white">{{ $record ? 'Save' : 'Add record' }}</button>
