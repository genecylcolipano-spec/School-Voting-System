...
<nav class="min-h-0 flex-1 space-y-1 overflow-y-auto overscroll-contain px-3 py-4" x-data="{
    openEvents: {{ $eventsActive ? 'true' : 'false' }},
    openTalent: {{ $talentActive ? 'true' : 'false' }},
    openVoting: {{ $votingActive ? 'true' : 'false' }},
    openResults: {{ $resultsActive ? 'true' : 'false' }},
    openFundraising: {{ $fundraisingActive ? 'true' : 'false' }},
    openComms: {{ $commsActive ? 'true' : 'false' }},
    openReports: {{ $reportsActive ? 'true' : 'false' }},
    openStudents: {{ $isSuperAdmin && $userManagementActive ? 'true' : 'false' }},
    openRoster: {{ $isSuperAdmin && $rosterManagementActive ? 'true' : 'false' }},
    openSystem: {{ $isSuperAdmin && $systemManagementActive ? 'true' : 'false' }},
}">
    {{-- Dashboard --}}
    <a href="{{ route($dashboardRoute) }}" @click="sidebarOpen = false" class="{{ $topLink($onDashboard) }}">
    ...
    {{-- Event Management --}}
    <div>
        <button type="button" @click="openEvents = !openEvents" class="{{ $groupBtn($eventsActive) }}">
            <svg class="h-5 w-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        ...
        <div x-show="openEvents && !collapsed" x-transition class="mt-1 space-y-0.5 pl-4">
            ...
            <div>
                ...
                <div x-show="openTalent" x-transition class="mt-0.5 space-y-0.5 border-l border-violet-500/10 pl-3">
                    <a href="{{ route('admin.talent-competition.index') }}" @click="sidebarOpen = false" class="{{ $subLink(request()->routeIs('admin.talent-competition.*')) }}">Competition Management</a>
                    <a href="{{ route('admin.talent-participants.index') }}" @click="sidebarOpen = false" class="{{ $subLink(request()->routeIs('admin.talent-participants.*')) }}">Participants</a>
                    <a href="{{ route('admin.live.talent') }}" @click="sidebarOpen = false" class="{{ $subLink(request()->routeIs('admin.live.talent')) }}">Live Monitoring</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div>
        ...
        <div x-show="openVoting && !collapsed" x-transition class="mt-1 space-y-0.5 pl-4">
            <a href="{{ route('admin.elections.index') }}" @click="sidebarOpen = false" class="{{ $subLink(request()->routeIs('admin.elections.*')) }}">Elections</a>
            <a href="{{ route('admin.campaigns.index') }}" @click="sidebarOpen = false" class="{{ $subLink(request()->routeIs('admin.campaigns.*')) }}">Campaigns</a>
            <a href="{{ route('admin.live.election') }}" @click="sidebarOpen = false" class="{{ $subLink(request()->routeIs('admin.live.election')) }}">Live Monitoring</a>
        </div>
    </div>
    ...
    <div>
        ...
        <div x-show="openFundraising && !collapsed" x-transition class="mt-1 space-y-0.5 pl-4">
            <a href="{{ route('admin.fundraisers.index') }}" @click="sidebarOpen = false" class="{{ $subLink(request()->routeIs('admin.fundraisers.*') && ! request()->routeIs('admin.fundraisers.donations', 'admin.fundraisers.transactions')) }}">Campaigns</a>
            <a href="{{ route('admin.fundraisers.donations') }}" @click="sidebarOpen = false" class="{{ $subLink(request()->routeIs('admin.fundraisers.donations')) }}">Donations</a>
    ...
    {{-- Communication --}}
    <div>
        <button type="button" @click="openComms = !openComms" class="{{ $groupBtn($commsActive) }}">
            <svg class="h-5 w-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        ...
        <div x-show="openComms && !collapsed" x-transition class="mt-1 space-y-0.5 pl-4">
            <a href="{{ route('admin.announcements.index') }}" @click="sidebarOpen = false" class="{{ $subLink(request()->routeIs('admin.announcements.*')) }}">Announcements</a>
            <a href="{{ route('admin.notifications.index') }}" @click="sidebarOpen = false" class="{{ $subLink(request()->routeIs('admin.notifications.*')) }}">Notifications</a>
        </div>
    </div>

    {{-- Reports & Analytics --}}
    <div>
        <button type="button" @click="openReports = !openReports" class="{{ $groupBtn($reportsActive) }}">
            <svg class="h-5 w-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
    ...
    {{-- User Management (registered system accounts only) --}}
    @if ($isSuperAdmin)
        <div>
            <button type="button" @click="openStudents = !openStudents" class="{{ $groupBtn($userManagementActive) }}">
                <svg class="h-5 w-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            ...
            <div x-show="openStudents && !collapsed" x-transition class="mt-1 space-y-0.5 pl-4">
                <a href="{{ route('admin.students.index') }}" @click="sidebarOpen = false" class="{{ $subLink(request()->routeIs('admin.students.*')) }}">Students</a>
                <a href="{{ route('super-admin.faculty.index') }}" @click="sidebarOpen = false" class="{{ $subLink(request()->routeIs('super-admin.faculty.*')) }}">Faculty</a>
                <a href="{{ route('super-admin.administrators.index') }}" @click="sidebarOpen = false" class="{{ $subLink(request()->routeIs('super-admin.administrators.*')) }}">Administrators</a>
            </div>
        </div>

        {{-- Roster Management (official institutional records) --}}
        <div>
            <button type="button" @click="openRoster = !openRoster" class="{{ $groupBtn($rosterManagementActive) }}">
                <svg class="h-5 w-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            ...
            <div x-show="openRoster && !collapsed" x-transition class="mt-1 space-y-0.5 pl-4">
                <a href="{{ route('super-admin.roster.students.index') }}" @click="sidebarOpen = false" class="{{ $subLink(request()->routeIs('super-admin.roster.students.*')) }}">Student Roster</a>
                <a href="{{ route('super-admin.roster.faculty.index') }}" @click="sidebarOpen = false" class="{{ $subLink(request()->routeIs('super-admin.roster.faculty.*')) }}">Faculty Roster</a>
                <a href="{{ route('super-admin.roster.administrators.index') }}" @click="sidebarOpen = false" class="{{ $subLink(request()->routeIs('super-admin.roster.administrators.*')) }}">Administrator Roster</a>
            </div>
        </div>

        {{-- System Management --}}
        <div>
            <button type="button" @click="openSystem = !openSystem" class="{{ $groupBtn($systemManagementActive) }}">
                <svg class="h-5 w-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
    ...
    @elseif ($canViewStudents)
        <a href="{{ route('admin.students.index') }}" @click="sidebarOpen = false" class="{{ $topLink(request()->routeIs('admin.students.*')) }}">
            <svg class="h-5 w-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
            </svg>
    ...
    {{-- Settings (personal account only — not platform/system settings) --}}
    <a href="{{ route('profile.edit') }}" @click="sidebarOpen = false" class="{{ $topLink(request()->routeIs('profile.*')) }}">
        <svg class="h-5 w-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
