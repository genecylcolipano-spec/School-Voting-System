@if (session('enrollment_url'))
    <div class="mb-4 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4" x-data="{ copied: false }">
        ...
        <a href="{{ session('enrollment_url') }}" class="mt-2 block break-all text-sm text-violet-300 hover:text-violet-200">
        ...
        <div class="mt-3 flex flex-wrap gap-2">
            <button type="button"
                ...
                @click="navigator.clipboard.writeText(@js(session('enrollment_url'))); copied = true; setTimeout(() => copied = false, 2000)">
                <span x-text="copied ? 'Copied!' : 'Copy link'"></span>
            </button>
            <a href="{{ session('enrollment_url') }}"
