...
<li class="space-y-2 px-5 py-3.5">
    <div class="flex items-center justify-between gap-3">
        <div class="min-w-0">
            <span class="text-sm text-slate-400">#{{ $rank }}</span>
            <span class="ml-2 font-medium text-white">{{ $name }}</span>
        </div>
        @if (! empty($meta))
            <span class="shrink-0 text-xs text-slate-500">{{ $meta }}</span>
        @endif
    </div>
    ...
    @if (isset($votes))
        <div class="flex items-center gap-3">
            <div
                ...
                aria-label="{{ $name }} received {{ number_format($percent, 1) }} percent of votes"
            >
                <div class="h-full rounded-full {{ $barClass }}" style="width: {{ number_format($percent, 1, '.', '') }}%"></div>
            </div>
            <span class="w-24 shrink-0 text-right text-xs font-semibold {{ $valueClass }}">
                {{ number_format((float) $votes, $voteDecimals) }} · {{ number_format($percent, 1) }}%
            </span>
        </div>
    @endif
</li>
