...
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($isEdit ? 'Edit Event' : 'Create Event'),'user' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($user),'notifications-count' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($notificationsCount)]); ?>
        <form method="POST" action="<?php echo e($isEdit ? route('admin.events.update', $event) : route('admin.events.store')); ?>" enctype="multipart/form-data" class="max-w-2xl min-w-0 space-y-4 rounded-2xl border border-cyan-500/15 bg-slate-900/70 p-4 sm:p-6">
...
<?php endif; ?>
            ...
            <div>
                ...
                <select name="status" class="mt-1 w-full rounded-xl border border-slate-700 bg-slate-950/50 px-4 py-2 text-slate-100 [color-scheme:dark]">
                    <?php $__currentLoopData = $statuses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $status): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($status->value); ?>" <?php if(old('status', optional($event)->status?->value) === $status->value): echo 'selected'; endif; ?>><?php echo e($status->label()); ?></option>
            ...
            <div class="flex flex-col gap-3 sm:flex-row">
                ...
                <a href="<?php echo e(route('admin.events.index')); ?>" class="inline-flex min-h-11 w-full items-center justify-center rounded-xl border border-slate-700 px-5 py-2.5 text-sm text-slate-300 sm:w-auto">Cancel</a>
            </div>
        </form>
