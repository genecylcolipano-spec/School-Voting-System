        ...
        <?php if($canPickElection ?? false): ?>
            <div class="mt-4">
                ...
                <select name="election_id" required class="mt-1 w-full rounded-xl border border-slate-700 bg-slate-950/50 px-4 py-2 text-slate-100">
                    <?php $__currentLoopData = $hostElections; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $host): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($host->id); ?>" <?php if((int) old('election_id', $election?->id) === (int) $host->id): echo 'selected'; endif; ?>>
                            <?php echo e($host->title); ?> (<?php echo e($host->status?->label() ?? 'Unknown'); ?>)
                        </option>
