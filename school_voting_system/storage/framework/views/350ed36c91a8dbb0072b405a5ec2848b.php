...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.admin-portal','data' => ['title' => ($record ? 'Edit ' : 'Add ') . $rosterLabel . ' Roster','user' => $user,'notificationsCount' => $notificationsCount]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(($record ? 'Edit ' : 'Add ') . $rosterLabel . ' Roster'),'user' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($user),'notifications-count' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($notificationsCount)]); ?>
        <?php echo $__env->make('admin.partials.page-header', [
            'title' => ($record ? 'Edit ' : 'Add ').$rosterLabel.' Roster Record',
            'description' => $record
                ? 'Update an official institutional roster row.'
                : 'Add one official institutional roster row. This does not create a login or send an enrollment email.',
            'showAction' => false,
        ], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>

        <section class="mx-auto max-w-2xl rounded-2xl border border-violet-500/15 bg-slate-900/70 p-5 sm:p-6">
            <form method="POST" action="<?php echo e($record ? route($routePrefix.'.update', $record) : route($routePrefix.'.store')); ?>" class="space-y-5">
                <?php echo csrf_field(); ?>
                <?php if($record): ?>
                ...
                <div>
                    ...
                    <?php if($accountIdLocked): ?>
                        <input value="<?php echo e($record->account_id); ?>" readonly disabled
                            class="mt-1 w-full cursor-not-allowed rounded-xl border border-slate-800 bg-slate-950/30 px-4 py-2.5 font-mono text-slate-400">
                        <p class="mt-1 text-xs text-slate-500">Account ID cannot be changed after this person has registered. It stays matched to their portal login.</p>
                    <?php else: ?>
                        <input name="account_id" value="<?php echo e(old('account_id', $record?->account_id)); ?>" required
                            class="mt-1 w-full rounded-xl border border-slate-700 bg-slate-950/50 px-4 py-2.5 font-mono text-white">
                    <?php endif; ?>
...
unset($__errorArgs, $__bag); ?>
                ...
                <div class="flex flex-wrap gap-3 pt-2">
                    <button type="submit" class="rounded-xl bg-gradient-to-r from-violet-600 to-indigo-500 px-5 py-2.5 text-sm font-semibold text-white"><?php echo e($record ? 'Save' : 'Add record'); ?></button>
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\voting system\school_voting_system\resources\views/admin/rosters/form.blade.php ENDPATH**/ ?>