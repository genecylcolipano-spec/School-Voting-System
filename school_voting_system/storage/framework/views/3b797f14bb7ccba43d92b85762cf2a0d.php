...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.student-portal','data' => ['title' => 'Results','user' => $user,'notificationsCount' => $notificationsCount]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $component->withAttributes(['title' => 'Results','user' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($user),'notifications-count' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($notificationsCount)]); ?>
        <section class="rounded-2xl border border-cyan-500/15 bg-slate-900/70 p-5 sm:p-6">
            <p class="text-sm text-slate-400">View official results of student elections and talent competitions after administrators publish them.</p>
        </section>

        <?php if(! $hasEvents): ?>
            <div class="flex flex-col items-center justify-center rounded-2xl border border-dashed border-cyan-500/20 bg-slate-900/50 px-6 py-16 text-center">
        ...
        <?php else: ?>
            <div class="space-y-4">
                <?php $__currentLoopData = $events; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $event): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <article class="rounded-2xl border border-cyan-500/15 bg-slate-900/70 p-5 sm:p-6">
                        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <div class="flex shrink-0 flex-wrap items-center gap-3 sm:flex-col sm:items-end lg:flex-row lg:items-center">
                                ...
                                <?php elseif($event['already_voted'] ?? false): ?>
                                    <span class="inline-flex items-center justify-center rounded-xl border border-emerald-500/25 bg-emerald-500/10 px-4 py-2 text-sm font-medium text-emerald-200">
                                        You have voted
                                    </span>
            ...
            <?php if(! $hasCompletedEvents): ?>
                <div class="flex flex-col items-center justify-center rounded-2xl border border-dashed border-cyan-500/15 bg-slate-900/40 px-6 py-10 text-center">
